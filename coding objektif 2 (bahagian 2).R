...
refit_sarimax <- ts_tbl_interv %>%
  model(
    sarimax_interv = ARIMA(y ~ pandemic_dummy)
  )
...
# Define scaling functions based on FULL data
...
for (i in 1:6) {
  current_batch_arr <- array_reshape(
    current_batch_data_2025, c(1, n_steps, n_features)
  )
  ...
  new_row <- c(
    pred_scaled,
    future_features_2025$pandemic_dummy[i],
    future_features_2025$month_sin[i],
    future_features_2025$month_cos[i]
  )
...
# Filter data from 2020 onwards for better visualization
...
p_comparison_2025 <- ggplot(history_plot_data, aes(x = date, y = y)) +
  ...
  geom_line(
    data = sarimax_fc_2025,
    aes(y = .mean, color = "Ramalan SARIMAX"),
  ...
  annotate(
    "text",
    x = vline_date,
    y = max(history_plot_data$y) * 0.95,
    label = "Permulaan Ramalan  ",
    hjust = 1.1,
    vjust = 1,
    color = "grey20",
    size = 3.5,
...
comparison_2025 %>%
  kable(format = "simple", align = c("l", "r", "r")) %>%
  print()
...
validation_df <- tibble(
  date = lstm_fc_2025$date,  # Ambil tarikh dari data ramalan sedia ada
  Actual = actual_values_2025,
  SARIMAX_Pred = sarimax_fc_2025$.mean,     # Nilai ramalan SARIMAX
  LSTM_Pred = lstm_fc_2025$forecast_lstm    # Nilai ramalan LSTM
)
...
validation_display <- validation_df %>%
  mutate(
    ...
    # Kira perbezaan peratusan (Ralat)
    `Ralat SARIMAX (%)` = sprintf("%.2f%%", abs((Actual - SARIMAX_Pred)/Actual)*100),
    `Ralat LSTM (%)` = sprintf("%.2f%%", abs((Actual - LSTM_Pred)/Actual)*100)
  ) %>%
  select(Bulan, Actual_Fmt, SARIMAX_Fmt, `Ralat SARIMAX (%)`, LSTM_Fmt, `Ralat LSTM (%)`) %>%
  rename(
    `Data Sebenar` = Actual_Fmt,
...
# Kira untuk kedua-dua model
acc_sarimax_2025 <- calc_accuracy_manual(validation_df$Actual, validation_df$SARIMAX_Pred, "SARIMAX")
acc_lstm_2025 <- calc_accuracy_manual(validation_df$Actual, validation_df$LSTM_Pred, "LSTM")
...
final_accuracy_table %>%
  kable(format = "simple", caption = "Prestasi Model bagi Januari - Jun 2025") %>%
  print()
...
p_validasi <- ggplot(plot_data_final, aes(x = date, y = Nilai, color = Jenis, linetype = Jenis)) +
  geom_line(linewidth = 1.2) +
  geom_point(size = 3) +
  scale_color_manual(values = c("Actual" = "black", "SARIMAX_Pred" = "red", "LSTM_Pred" = "blue"),
                     labels = c("Data Sebenar", "Ramalan LSTM", "Ramalan SARIMAX")) +
  scale_linetype_manual(values = c("Actual" = "solid", "SARIMAX_Pred" = "dashed", "LSTM_Pred" = "dashed"),
                        labels = c("Data Sebenar", "Ramalan LSTM", "Ramalan SARIMAX")) +
